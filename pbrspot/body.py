...
import pbrspot
from pbrspot.joint import Joint
from pbrspot.link import Link
...
def createBody(path, **kwargs):
    with pbrspot.helper.HideOutput():
        with pbrspot.utils.LockRenderer():
            body_id = pbrspot.utils.load_model(path, **kwargs)
...
class Body(object):
    ...
    def remove_body(self):
        if (CLIENT, self.id) in pbrspot.utils.INFO_FROM_BODY:
            del pbrspot.utils.INFO_FROM_BODY[CLIENT, self.id]
    ...
    def get_transform(self):
        return pbrspot.geometry.tform_from_pose(self.get_pose())
    ...
    def get_euler(self):
        return pbrspot.geometry.euler_from_quat(self.get_quat())

    def get_base_values(self):
        return pbrspot.utils.base_values_from_pose(self.get_pose())
    ...
    def set_transform(self, transform):
        self.set_pose(pbrspot.geometry.pose_from_tform(transform))
    ...
    def set_euler(self, euler):
        self.set_quat(pbrspot.geometry.quat_from_euler(euler))

    def set_base_values(self, values):
        _, _, z = self.get_point()
        x, y, theta = values
        self.set_point((x, y, z))
        self.set_quat(pbrspot.geometry.z_rotation(theta))
    ...
    def get_custom_limits(self, joints=None, custom_limits={}, circular_limits=pbrspot.utils.UNBOUNDED_LIMITS):
        joint_limits = []
    ...
    def get_relative_pose(self, link1, link2):
        ...
        link2_from_link1 = pbrspot.geometry.multiply(pbrspot.geometry.invert(world_from_link2), world_from_link1)
    ...
    def dump_body(self):
        ...
        print('Link id: {} | Name: {} | Mass: {} | Collision: {} | Visual: {}'.format(
            ...
            len(pbrspot.utils.get_collision_data(self, self.base_link)), -1)) # len(get_visual_data(body, link))))
        ...
        for link in self.links:
            ...
            print('Link id: {} | Name: {} | Joint: {} | Parent: {} | Mass: {} | Collision: {} | Visual: {}'.format(
                ...
                len(pbrspot.utils.get_collision_data(self, link.linkID)), -1)) # len(get_visual_data(body, link)))) #XXX move this function from utils
