...
def get_collision_fn(body, joints, obstacles, attachments, self_collisions, disabled_collisions,
    ...
    def collision_fn(q):
        if not pbrspot.helper.all_between(lower_limits, q, upper_limits):
        ...
        for link1, link2 in check_link_pairs:
            ...
            if pbrspot.collisions.pairwise_link_collision(body, link1, body, link2): #, **kwargs):
                #print(get_body_name(body), get_link_name(body, link1), get_link_name(body, link2))
                return True
        for body1, body2 in check_body_pairs:
            if pbrspot.collisions.pairwise_collision(body1, body2, **kwargs):
...
def plan_base_motion(body, end_conf, base_limits, obstacles=[], direct=False,
    ...
    def collision_fn(q):
        ...
        return any(pbrspot.collisions.pairwise_collision(body, obs, max_distance=max_distance) for obs in obstacles)
...
def plan_cartesian_motion(robot, first_joint, target_link, waypoint_poses,
    ...
    sub_robot = pbrspot.utils.clone_body(robot, links=selected_links, visual=False, collision=False) # TODO: joint limits
    ...
    for target_pose in waypoint_poses:
        for iteration in range(max_iterations):
            sub_kinematic_conf = pbrspot.utils.inverse_kinematics_helper(sub_robot, selected_target_link, target_pose, null_space=null_space)
            if sub_kinematic_conf is None:
                sub_robot.remove_body()
                return None
            sub_robot.set_joint_positions(sub_movable_joints, sub_kinematic_conf)
            if pbrspot.utils.is_pose_close(sub_robot.get_link_pose(selected_target_link), target_pose, **kwargs):
                robot.set_joint_positions(selected_movable_joints, sub_kinematic_conf)
                kinematic_conf = robot.get_configuration()
                if not pbrspot.helper.all_between(lower_limits, kinematic_conf, upper_limits):
