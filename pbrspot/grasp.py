...
def create_attachment(parent, parent_link, child):
    ...
    grasp_pose = pbrspot.geometry.multiply(pbrspot.geometry.invert(parent_link_pose), child_pose)
...
def body_from_end_effector(end_effector_pose, grasp_pose):
    ...
    """
    return pbrspot.geometry.multiply(end_effector_pose, grasp_pose)
...
def end_effector_from_body(body_pose, grasp_pose):
    ...
    """
    return pbrspot.geometry.multiply(body_pose, pbrspot.geometry.invert(grasp_pose))

def approach_from_grasp(approach_pose, end_effector_pose):
    return pbrspot.geometry.multiply(approach_pose, end_effector_pose)
...
def get_grasp_pose(constraint):
    ...
    return pbrspot.geometry.multiply(pbrspot.geometry.invert(joint_from_parent), joint_from_child)
...
def add_fixed_constraint(body, robot, robot_link, max_force=None):
    ...
    grasp_pose = pbrspot.geometry.multiply(pbrspot.geometry.invert(end_effector_pose), body_pose)
    ...
    constraint = p.createConstraint(robot, robot_link, body, body_link,  # Both seem to work
                                    p.JOINT_FIXED, jointAxis=pbrspot.geometry.unit_point(),
                                    parentFramePosition=point,
                                    childFramePosition=pbrspot.geometry.unit_point(),
                                    parentFrameOrientation=quat,
                                    childFrameOrientation=pbrspot.geometry.unit_quat(),
