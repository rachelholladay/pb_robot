...
def stable_z_on_aabb(body, aabb):
    center, extent = pb_robot_spot.aabb.get_center_extent(body)
    _, upper = aabb
    return (upper + extent/2 + (body.get_point() - center))[2]

def stable_z(body, surface, surface_link=None):
    return stable_z_on_aabb(body, pb_robot_spot.aabb.get_aabb(surface, link=surface_link))

def is_placed_on_aabb(body, bottom_aabb, above_epsilon=1e-2, below_epsilon=0.0):
    top_aabb = pb_robot_spot.aabb.get_aabb(body) # TODO: approximate_as_prism
    bottom_z_max = bottom_aabb[1][2]
    top_z_min = top_aabb[0][2]
    return ((bottom_z_max - below_epsilon) <= top_z_min) and \
           (top_z_min <= (bottom_z_max + above_epsilon)) and \
           (pb_robot_spot.aabb.aabb_contains_aabb(pb_robot_spot.aabb.aabb2d_from_aabb(top_aabb), pb_robot_spot.aabb.aabb2d_from_aabb(bottom_aabb)))

def is_placement(body, surface, **kwargs):
    return is_placed_on_aabb(body, pb_robot_spot.aabb.get_aabb(surface), **kwargs)

def sample_placement_on_aabb(top_body, bottom_aabb, top_pose=pb_robot_spot.geometry.unit_pose(),
    ...
    for _ in range(max_attempts):
        ...
        rotation = pb_robot_spot.geometry.Euler(yaw=theta)
        top_body.set_pose(pb_robot_spot.geometry.multiply(pb_robot_spot.geometry.Pose(euler=rotation), top_pose))
        center, extent = pb_robot_spot.aabb.get_center_extent(top_body)
        ...
        pose = pb_robot_spot.geometry.multiply(pb_robot_spot.geometry.Pose(point, rotation), top_pose)
...
def sample_placement(top_body, bottom_body, bottom_link=None, **kwargs):
    bottom_aabb = pb_robot_spot.aabb.get_aabb(bottom_body, link=bottom_link)
...
def is_center_on_aabb(body, bottom_aabb, above_epsilon=1e-2, below_epsilon=0.0):
    ...
    center, extent = pb_robot_spot.aabb.get_center_extent(body) # TODO: approximate_as_prism
    ...
    return ((bottom_z_max - abs(below_epsilon)) <= top_z_min <= (bottom_z_max + abs(above_epsilon))) and \
           (pb_robot_spot.aabb.aabb_contains_point(base_center[:2], pb_robot_spot.aabb.aabb2d_from_aabb(bottom_aabb)))

def is_center_stable(body, surface, **kwargs):
    return is_center_on_aabb(body, pb_robot_spot.aabb.get_aabb(surface), **kwargs)
